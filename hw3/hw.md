...
- линтер код-стайла;
- покрытие тестами >= 80%;
- линтер событий по schema registry;
- алерты по SLA (мониторинг доступности).
...
- нагрузочные тесты для проверки scalability и elascticity;
- проверка, что плагины не вносят изменения в core;
- CI-проверка релизного цикла;
- автотесты на новые плагины.

### Матчинг:
- проверка реализации архитектурного стиля. Как это делать — узнать у разработчиков. Не получится автоматизировать — проверять вручную;
- нагрузочные тесты для проверки scalability и elascticity;
- автотесты на валидность пошагового алгоритма.

### Сборка расходников:
- алерты и метрики на долгую сборку расходников.

### Управление заказами и контроль качества:
- алерты и метрики на *висящие заказы*;

### Выплаты воркерам и оплата клиентами:
- автотест сверки инвойса и транзакций;
- алерт на оплату/выплату, которые завершились с ошибкой;
- алерт на расхождение балансов.

ADR
------

[ADR-001: Обеспечение высокого релизного цикла и гибкости сервиса найма.](./adrs/hiring/adr-001-hiring-high-deployability-and-agility.md)
